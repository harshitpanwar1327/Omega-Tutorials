        ...
        {menuOpen && (
          ...
          >
            ...
            <div className="lg:hidden flex flex-col items-center gap-1 text-gray-700 font-medium">
              <NavLink to="/" className={navClass} onClick={()=>setMenuOpen(!menuOpen)}>Home</NavLink>
              <NavLink to="/about-us" className={navClass} onClick={()=>setMenuOpen(!menuOpen)}>About Us</NavLink>
              <NavLink to="/courses" className={navClass} onClick={()=>setMenuOpen(!menuOpen)}>Courses</NavLink>
              <NavLink to="/results" className={navClass} onClick={()=>setMenuOpen(!menuOpen)}>Results</NavLink>
              <NavLink to="/gallery" className={navClass} onClick={()=>setMenuOpen(!menuOpen)}>Gallery</NavLink>
              <NavLink to="/contact" className={navClass} onClick={()=>setMenuOpen(!menuOpen)}>Contact</NavLink>
              <NavLink to="/contact" className="mt-3 w-full text-center bg-[linear-gradient(135deg,hsl(215_85%_45%),hsl(205_80%_60%))] text-white px-4 py-2 rounded-xl font-semibold shadow-[0_2px_20px_-4px_hsl(215_85%_45%/0.12)] hover:scale-102 transition duration-300" onClick={()=>setMenuOpen(!menuOpen)}
              >Enroll Now</NavLink>
            </div>
          </motion.div>
        )}
